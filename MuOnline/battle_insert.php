...
<div id="battle_report_title"><b>Battle Report</b></div>
<div id="js_output">
<?php
...
<?php
echo '<div id="turns"><b>Turn</b><br/>'.$_SESSION['player_stats']['turns'].'</div>';
if(isset($_GET['note']))
...
echo '<div class="defeated_name">Defeated '.$_SESSION['enemies'][$i]['name'].'</div>';
echo '<div class="defeated_exp">EXP: '.$_SESSION['enemies'][$i]['exp'].'</div>';
echo '<div class="defeated_exp">Zen: '.$_SESSION['enemies'][$i]['zen'].'</div><br/>';
...
if($alive<1)
echo '<div id="enemy_buttons"><button type="button" id="save_cont">Next Battle</button>&nbsp;<button type="button" id="save_exit">Leave Battle</button></div>';
}
?>
</div>
<div id="newui">
...
$('#js_output').text('Not enough space in your inventory!');
else
$('#js_output').text('Inventory Full');
}
else
{
$('#js_output').text('The item has been added to your inventory.');
$('#'+item_id).parent().html('[Took the item]');
}
    }
  }
...
$('.defeated_item_ins').click(function(){
 nd();
 item_id = this.id;
 jConfirm('Add this item to your inventory?', 'Confirm Adding', function(r) {adding_confirmed(r, item_id);});
});
